...
GROUP BY `deposit_group` ORDER BY MIN(`magic_wand_size`) LIMIT 1;
...
FROM `wizzard_deposits` GROUP BY `deposit_group`, `magic_wand_creator`
...
GROUP BY `age_group` ORDER BY `wizard_count`;
...
-- SELECT ((SELECT `deposit_amount` FROM `wizzard_deposits` LIMIT 1) - 
